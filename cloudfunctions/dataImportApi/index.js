...
function normalizeProduct(input) {
  ...
  if (!hasOwn(input, 'priceYuan') || text(input.priceYuan) === '') errors.push('价格不能为空')
  else if (number(input.priceYuan, -1) < 0) errors.push('价格不能小于 0')
  ...
  data.name = name
  data.type = type
  if (hasOwn(input, 'priceYuan') && text(input.priceYuan) !== '') {
...
function buildDocument(type, rowData, existing, adminContext, jobId) {
  ...
  if (type === 'products') {
    ...
    ;['category', 'subtitle', 'unit', 'coverFileId'].forEach((key) => setText(key))
    ;['sceneTags', 'colorTags', 'searchKeywords', 'atlasCodes', 'atlasIds'].forEach(setArray)
