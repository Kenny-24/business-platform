...
Page({
  ...
  async syncUserState() {
    ...
    const validAtlasIds = new Set(
      (this.data.allItems || []).map((item) => String(item.id || ''))
    )
    const validFavorites = (ids) => stringArray(ids).filter((id) => validAtlasIds.has(id))

    if (!getOverview) {
      favoriteIds = mergeFavoriteAtlasIds(
        validFavorites(localFavoriteIds)
      )
      this.setData({
        purchasedIds:
    ...
    try {
      ...
      favoriteIds = mergeFavoriteAtlasIds(
        validFavorites(
          cached && cached.favoriteAtlasIds
