  ...
  <div class="data-import-page">
    ...
    <el-card v-if="validation" shadow="never" class="panel-card validation-panel">
      ...
      <el-table :data="validation.rows" row-key="rowNo" max-height="520" class="clean-table import-preview-table">
        ...
        >
          <template #default="{ row }">
            {{ formatPreviewColumn(row, column) }}
          </template>
        </el-table-column>
...
function formatPreviewColumn(row, column) {
  if (column.key === 'priceYuan' && row.data?.priceFen !== undefined) {
    return formatPreviewValue(Number(row.data.priceFen || 0) / 100)
  }
  return formatPreviewValue(row.data?.[column.key])
}
...
async function commitCurrent() {
  ...
  try {
    ...
    const sourceRowsByNumber = new Map(
      parsedRows.value.map((row) => [Number(row.rowNo), row])
    )
    const commitRows = validation.value.rows.map((row) => {
      const sourceRow = sourceRowsByNumber.get(Number(row.rowNo)) || {}
      const data = { ...sourceRow, rowNo: row.rowNo }
      const normalizedData = row.data || {}
      const fileIdField = schema.value.imageFileIdField
      const fileNameField = schema.value.imageFileNameField

      if (fileIdField && normalizedData[fileIdField]) {
        data[fileIdField] = normalizedData[fileIdField]
      }

      const fileName = baseName(data[fileNameField])
      if (fileName && imageMap.has(fileName)) {
        data[fileIdField] = imageMap.get(fileName)
      }
      return data
    })
