...
Page({
  ...
  onShow() {
    ...
    const now = Date.now()
    const couponUpdatedAt = Number(wx.getStorageSync(COUPON_UPDATE_STORAGE_KEY) || 0)
    if (
      couponUpdatedAt > Number(this._lastCouponLoadAt || 0) ||
      now - Number(this._lastCouponLoadAt || 0) > 60000
    ) {
      this.scheduleCouponEntryLoad()
    }
  ...
  onUnload() {
    ...
    clearTimeout(this._couponEntryTimer)
    this._couponEntryTimer = null
  },
  ...
  scheduleCouponEntryLoad() {
    clearTimeout(this._couponEntryTimer)
    this._couponEntryTimer = setTimeout(() => {
      this.loadCouponEntry()
    }, 160)
  },

  async loadCouponEntry() {
    if (this._couponLoading) return
    this._couponLoading = true
    try {
      const result = await listClaimableCoupons()
      const items = result.items || []
      const available = items.find((item) => item.canClaim)
      const owned = items.find((item) => item.claimState === 'claimed')
      const couponEntry = available
        ? {
            title: available.name,
            subtitle: `${available.thresholdText} · ${available.validityText}`,
            actionText: '立即领取'
          }
        : owned
          ? {
              title: '领券中心',
              subtitle: '查看当前开放的优惠活动',
              actionText: '查看活动'
            }
          : {
              title: '领取专属优惠券',
      ...
      this._lastCouponLoadAt = Date.now()
      this.setData({ couponEntry })
    } catch (error) {
      console.warn('首页优惠券入口加载失败：', error)
    } finally {
