...
Page({
  ...
  async loadCalendarData(forceRefresh = false) {
    ...
    try {
      ...
      this.setData({
        holidayEvents: (data.calendarEvents || []).filter((item) => item.region !== 'merchant').length
          ? (data.calendarEvents || []).filter((item) => item.region !== 'merchant')
          : cloneHolidayCatalog().filter((item) => item.region !== 'merchant').map((item) => ({
              ...item,
              enabled: true,
              builtIn: true
            })),
      ...
      this.setData({
        holidayEvents: cloneHolidayCatalog().filter((item) => item.region !== 'merchant').map((item) => ({
          ...item,
          enabled: true,
          builtIn: true
        })),
