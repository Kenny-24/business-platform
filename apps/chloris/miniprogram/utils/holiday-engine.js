...
function eventMatchesDate(event, date) {
  ...
  if (rule.type === 'date') {
    return dateKey(year, month, day) === String(rule.date || '')
  }

  if (rule.type === 'dateRange') {
    const current = dateKey(year, month, day)
    const start = String(rule.startDate || '')
    const end = String(rule.endDate || start)
    return Boolean(start && current >= start && current <= end)
  }
...
function normalizePersonalDate(item) {
  return {
    ...
    searchKeywords: Array.isArray(item.searchKeywords)
      ? item.searchKeywords
      : [],
    categoryIntent: item.categoryPreference || '推荐花束',
    sort: 2000
  }
}
...
function formatRule(rule) {
  ...
  if (rule.type === 'date') return String(rule.date || '未设置日期')
  if (rule.type === 'dateRange') {
    const start = String(rule.startDate || '')
    const end = String(rule.endDate || start)
