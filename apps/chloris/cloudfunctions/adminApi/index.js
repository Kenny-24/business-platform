...
function sanitizeProduct(input) {
  ...
  return {
    ...
    reservationDeadlineAt: text(input.reservationDeadlineAt),
...
async function listCalendarEvents() {
  ...
  return {
    items: mergeCalendarEvents(overrides).filter((item) => text(item.region) !== 'merchant'),
...
function sanitizeCalendarEvent(input) {
  ...
  const region = ['domestic', 'international'].includes(requestedRegion)
    ? requestedRegion
...
function orderView(item, urlMap = {}) {
  ...
  return {
    ...
    coupon: text(item.userCouponId) ? {
      userCouponId: text(item.userCouponId),
      couponId: text(item.couponId),
      name: text(item.couponName, '优惠券'),
      code: text(item.couponCode),
      discountFen: integer(item.discountFen),
...
const COUPON_CATEGORY_OPTIONS = new Set([
  'flower', 'bouquet', 'succulent', 'greenPlant', 'vase', 'gift', 'tool'
])
...
function couponAllowsPublicClaim(item) {
  if (item && Object.prototype.hasOwnProperty.call(item, 'publicClaimable')) {
    return item.publicClaimable === true
  }
  return LEGACY_PUBLIC_CLAIM_COUPON_CODES.has(text(item && item.code).toUpperCase())
}

function couponTemplateView(item, issued = []) {
  const issuedItems = issued.filter((row) => text(row.couponId) === text(item._id))
  const usedCount = issuedItems.filter((row) => text(row.status) === 'used').length
  const activeCount = issuedItems.filter((row) => ['unused', 'locked'].includes(text(row.status))).length
  const targetLabels = {
    all: '全部顾客',
    neverPurchased: '注册未购买',
    inactive30: '30天未购买',
    inactive60: '60天未购买',
    inactive90: '90天未购买',
    specificUser: '指定顾客'
  ...
  return {
    ...
    code: text(item.code),
    name: text(item.name),
    thresholdFen: integer(item.thresholdFen),
    discountFen: integer(item.discountFen),
    thresholdText: integer(item.thresholdFen) > 0 ? `满¥${formatFen(item.thresholdFen)}` : '无门槛',
    discountText: formatFen(item.discountFen),
    targetType: text(item.targetType, 'all'),
    targetTypeLabel: targetLabels[text(item.targetType, 'all')] || text(item.targetType),
    autoTrigger: text(item.autoTrigger, 'none'),
    autoTriggerLabel: triggerLabels[text(item.autoTrigger, 'none')] || text(item.autoTrigger),
    publicClaimable: couponAllowsPublicClaim(item),
    validityType: text(item.validityType, 'relative'),
    applicableCategories: stringArray(item.applicableCategories),
    excludedCategories: stringArray(item.excludedCategories),
    excludedCampaignTypes: stringArray(item.excludedCampaignTypes),
    issuedCount: issuedItems.length,
    usedCount,
    activeCount,
    remainingCount: integer(item.totalLimit) > 0 ? Math.max(0, integer(item.totalLimit) - issuedItems.length) : 0,
...
function sanitizeCouponTemplate(input) {
  const name = text(input.name)
  if (!name) throw new BusinessError('优惠券名称不能为空')
  const discountFen = integer(input.discountFen)
  const thresholdFen = integer(input.thresholdFen)
  if (discountFen <= 0) throw new BusinessError('优惠金额必须大于0')
  if (thresholdFen > 0 && discountFen >= thresholdFen) {
    throw new BusinessError('优惠金额应小于使用门槛')
  }
  const validityType = ['relative', 'fixed'].includes(text(input.validityType)) ? text(input.validityType) : 'relative'
  const startsAt = text(input.startsAt)
  const endsAt = text(input.endsAt)
  if (validityType === 'fixed') {
    if (!startsAt || !endsAt) throw new BusinessError('固定日期优惠券必须填写开始和结束时间')
    if (normalizedTimestamp(startsAt) >= normalizedTimestamp(endsAt)) throw new BusinessError('优惠券结束时间必须晚于开始时间')
  }
  const applicableCategories = stringArray(input.applicableCategories).filter((item) => COUPON_CATEGORY_OPTIONS.has(item))
  const excludedCategories = stringArray(input.excludedCategories).filter((item) => COUPON_CATEGORY_OPTIONS.has(item))
  const publicClaimable = boolean(input.publicClaimable)
  const targetType = publicClaimable
    ? 'all'
    : (['all', 'neverPurchased', 'inactive30', 'inactive60', 'inactive90', 'specificUser'].includes(text(input.targetType)) ? text(input.targetType) : 'all')
  const autoTrigger = publicClaimable
    ? 'none'
    : (['none', 'newUser', 'orderCompleted'].includes(text(input.autoTrigger)) ? text(input.autoTrigger) : 'none')
  return {
    code: text(input.code).toUpperCase(),
    name,
    ...
    discountFen,
    validityType,
    validDays: Math.max(1, integer(input.validDays, 7)),
    startsAt,
    endsAt,
    applicableCategories,
    excludedCategories,
    excludedCampaignTypes: stringArray(input.excludedCampaignTypes),
    totalLimit: integer(input.totalLimit),
    perUserLimit: Math.max(1, integer(input.perUserLimit, 1)),
    maxActivePerUser: Math.max(0, integer(input.maxActivePerUser)),
    firstOrderOnly: boolean(input.firstOrderOnly),
    maxBudgetFen: integer(input.maxBudgetFen),
    targetType,
    autoTrigger,
    publicClaimable,
    surprise: publicClaimable ? false : boolean(input.surprise),
    enabled: boolean(input.enabled),
    sort: integer(input.sort, 100)
  }
}
...
async function listCoupons() {
  const [templates, issued] = await Promise.all([
    safeGetAll(COLLECTIONS.coupons),
    safeGetAll(COLLECTIONS.userCoupons)
  ])
  return {
    items: templates
      .sort((a, b) => integer(b.sort) - integer(a.sort) || dateValue(b.updatedAt) - dateValue(a.updatedAt))
      .map((item) => couponTemplateView(item, issued)),
    total: templates.length,
    collectionReady: await collectionExists(COLLECTIONS.coupons),
    userCouponCollectionReady: await collectionExists(COLLECTIONS.userCoupons)
  }
}

async function saveCoupon(event, adminContext) {
  await Promise.all([
    assertCollectionExists(COLLECTIONS.coupons),
    assertCollectionExists(COLLECTIONS.userCoupons)
  ])
  const input = event.coupon || {}
  const data = sanitizeCouponTemplate(input)
  const id = text(input._id) || createId('coupon')
  let existing = null
  try { existing = firstDocument(await db.collection(COLLECTIONS.coupons).doc(id).get()) } catch (error) {}
  data.code = data.code || text(existing && existing.code) || defaultBusinessCode('CPN', id)
  await assertUniqueCode(COLLECTIONS.coupons, 'code', data.code, id)
  const document = {
    ...data,
    createdAt: existing && existing.createdAt || new Date(),
    updatedAt: new Date(),
  ...
  await db.collection(COLLECTIONS.coupons).doc(id).set(document)
  return { _id: id, ...document }
}

async function deleteCoupon(event) {
  await assertCollectionExists(COLLECTIONS.coupons)
  const id = text(event.id)
  if (!id) throw new BusinessError('缺少优惠券 ID')
  const issued = (await safeGetAll(COLLECTIONS.userCoupons)).some((item) => text(item.couponId) === id)
  if (issued) {
    await db.collection(COLLECTIONS.coupons).doc(id).update({ enabled: false, updatedAt: new Date() })
    return { _id: id, disabled: true }
  }
  await db.collection(COLLECTIONS.coupons).doc(id).remove()
  return { _id: id, removed: true }
}

function couponValidity(template, now = new Date()) {
  if (text(template.validityType, 'relative') === 'fixed') {
    return {
      validFrom: template.startsAt ? new Date(template.startsAt) : now,
      expiresAt: template.endsAt ? new Date(template.endsAt) : null
    }
  }
  const validDays = Math.max(1, integer(template.validDays, 7))
  return {
    validFrom: now,
...
async function issueCouponDocument(template, userId, source, issuedDocuments = []) {
  const now = Date.now()
  const userDocuments = issuedDocuments.filter((item) => text(item.userId) === userId)
  const existingCount = userDocuments.filter((item) => text(item.couponId) === text(template._id)).length
  if (existingCount >= Math.max(1, integer(template.perUserLimit, 1))) return false
  const maxActivePerUser = integer(template.maxActivePerUser)
  if (maxActivePerUser > 0) {
    const activeCount = userDocuments.filter((item) => {
      if (text(item.couponId) !== text(template._id)) return false
      if (!['unused', 'locked'].includes(text(item.status, 'unused'))) return false
      const expiresAt = dateValue(item.expiresAt)
      return !expiresAt || expiresAt > now
    }).length
    if (activeCount >= maxActivePerUser) return false
  }

  // 不定期惊喜券：同一顾客30天最多1张、滚动90天最多3张；
  // 已有同等或更高优惠的有效券时跳过，避免过度发券和重复优惠。
  if (template.surprise === true) {
    const surpriseDocuments = userDocuments.filter((item) => text(item.issueSource) === 'surprise')
    const issuedWithin30Days = surpriseDocuments.some((item) => {
      const issuedAt = dateValue(item.issuedAt || item.createdAt)
      return issuedAt > 0 && issuedAt >= now - 30 * 24 * 60 * 60 * 1000
    })
    if (issuedWithin30Days) return false
    const issuedWithin90Days = surpriseDocuments.filter((item) => {
      const issuedAt = dateValue(item.issuedAt || item.createdAt)
      return issuedAt > 0 && issuedAt >= now - 90 * 24 * 60 * 60 * 1000
    }).length
    if (issuedWithin90Days >= 3) return false
    const hasEqualOrBetterActiveCoupon = userDocuments.some((item) => {
      if (!['unused', 'locked'].includes(text(item.status, 'unused'))) return false
      const expiresAt = dateValue(item.expiresAt)
      if (expiresAt > 0 && expiresAt <= now) return false
      return integer(item.discountFen) >= integer(template.discountFen)
    })
    if (hasEqualOrBetterActiveCoupon) return false
  }

  const currentIssued = issuedDocuments.filter((item) => text(item.couponId) === text(template._id)).length
  if (integer(template.totalLimit) > 0 && currentIssued >= integer(template.totalLimit)) return false
  if (integer(template.maxBudgetFen) > 0 && currentIssued * integer(template.discountFen) >= integer(template.maxBudgetFen)) return false
  const { validFrom, expiresAt } = couponValidity(template, new Date(now))
  ...
  const document = {
    ...
    thresholdFen: integer(template.thresholdFen),
    discountFen: integer(template.discountFen),
    ...
    status: 'unused',
    issueSource: template.surprise === true ? 'surprise' : source,
    validFrom,
    expiresAt,
    issuedAt: new Date(),
    createdAt: new Date(),
  ...
  await db.collection(COLLECTIONS.userCoupons).doc(id).set(document)
  issuedDocuments.push({ _id: id, ...document })
  return true
}

function targetUsersForCoupon(targetType, users, orders, specificUserId = '') {
  const now = Date.now()
  const latestPurchase = new Map()
  const purchaseCount = new Map()
  for (const order of orders) {
    if (!['making', 'delivering', 'completed'].includes(text(order.status))) continue
    const userId = text(order.userId)
    if (!userId) continue
    purchaseCount.set(userId, (purchaseCount.get(userId) || 0) + 1)
    latestPurchase.set(userId, Math.max(latestPurchase.get(userId) || 0, dateValue(order.paidAt || order.createdAt)))
  }
  if (targetType === 'specificUser') return users.filter((user) => text(user._id) === specificUserId)
  if (targetType === 'neverPurchased') return users.filter((user) => !purchaseCount.get(text(user._id)))
  const dayMap = { inactive30: 30, inactive60: 60, inactive90: 90 }
  if (dayMap[targetType]) {
    const threshold = now - dayMap[targetType] * 24 * 60 * 60 * 1000
    return users.filter((user) => {
      const last = latestPurchase.get(text(user._id)) || dateValue(user.createdAt)
      return last > 0 && last <= threshold
    })
  }
  return users
}

async function issueCoupon(event, adminContext) {
  await Promise.all([
    assertCollectionExists(COLLECTIONS.coupons),
    assertCollectionExists(COLLECTIONS.userCoupons),
    assertCollectionExists(COLLECTIONS.users)
  ])
  const id = text(event.id)
  const template = firstDocument(await db.collection(COLLECTIONS.coupons).doc(id).get())
  if (!template || template.enabled !== true) throw new BusinessError('优惠券不存在或已停用')
  const targetType = text(event.targetType, template.targetType || 'all')
  const specificUserId = text(event.userId)
  const [users, orders, issuedDocuments] = await Promise.all([
    safeGetAll(COLLECTIONS.users),
    safeGetAll(COLLECTIONS.orders),
    safeGetAll(COLLECTIONS.userCoupons)
  ])
  const targets = targetUsersForCoupon(targetType, users.filter((user) => user.enabled !== false), orders, specificUserId)
  let issuedCount = 0
  let skippedCount = 0
  for (const user of targets.slice(0, 1000)) {
    const issued = await issueCouponDocument(template, text(user._id), template.surprise ? 'surprise' : 'manual', issuedDocuments)
  ...
  return { couponId: id, targetCount: targets.length, issuedCount, skippedCount }
}

const RECOMMENDED_COUPON_TEMPLATES = [
  { code: 'WELCOME99_15', name: '新客满99减15', description: '新用户首单专享，领取后7天有效。', thresholdFen: 9900, discountFen: 1500, validityType: 'relative', validDays: 7, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 1, maxActivePerUser: 1, maxBudgetFen: 0, targetType: 'neverPurchased', autoTrigger: 'newUser', publicClaimable: false, firstOrderOnly: true, surprise: false, enabled: true, sort: 900 },
  { code: 'DAILY199_20', name: '日常满199减20', description: '普通商品可用，领取后15天有效。', thresholdFen: 19900, discountFen: 2000, validityType: 'relative', validDays: 15, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 20, maxActivePerUser: 2, maxBudgetFen: 0, targetType: 'all', autoTrigger: 'none', publicClaimable: true, firstOrderOnly: false, surprise: false, enabled: true, sort: 800 },
  { code: 'DAILY299_35', name: '日常满299减35', description: '普通商品可用，领取后15天有效。', thresholdFen: 29900, discountFen: 3500, validityType: 'relative', validDays: 15, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 20, maxActivePerUser: 2, maxBudgetFen: 0, targetType: 'all', autoTrigger: 'none', publicClaimable: true, firstOrderOnly: false, surprise: false, enabled: true, sort: 790 },
  { code: 'REORDER199_25', name: '复购满199减25', description: '订单完成后自动发放，30天内有效。', thresholdFen: 19900, discountFen: 2500, validityType: 'relative', validDays: 30, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 999, maxActivePerUser: 2, maxBudgetFen: 0, targetType: 'all', autoTrigger: 'orderCompleted', publicClaimable: false, firstOrderOnly: false, surprise: false, enabled: true, sort: 780 },
  { code: 'SERVICE69_10', name: '售后补偿满69减10', description: '用于一般售后补偿，由后台指定顾客发放。', thresholdFen: 6900, discountFen: 1000, validityType: 'relative', validDays: 30, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 999, maxActivePerUser: 0, maxBudgetFen: 0, targetType: 'specificUser', autoTrigger: 'none', publicClaimable: false, firstOrderOnly: false, surprise: false, enabled: true, sort: 700 },
  { code: 'SERVICE199_20', name: '售后补偿满199减20', description: '用于较高客单订单的售后补偿，由后台指定顾客发放。', thresholdFen: 19900, discountFen: 2000, validityType: 'relative', validDays: 30, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 0, perUserLimit: 999, maxActivePerUser: 0, maxBudgetFen: 0, targetType: 'specificUser', autoTrigger: 'none', publicClaimable: false, firstOrderOnly: false, surprise: false, enabled: true, sort: 690 },
  { code: 'SURPRISE199_25', name: '不定期惊喜满199减25', description: '定向召回使用，72小时有效；同一顾客30天最多获得1张。', thresholdFen: 19900, discountFen: 2500, validityType: 'relative', validDays: 3, applicableCategories: [], excludedCategories: [], excludedCampaignTypes: ['limited','valentine','festival','seasonal','customQuote'], totalLimit: 300, perUserLimit: 999, maxActivePerUser: 1, maxBudgetFen: 750000, targetType: 'inactive30', autoTrigger: 'none', publicClaimable: false, firstOrderOnly: false, surprise: true, enabled: true, sort: 850 }
]

async function seedCoupons(event, adminContext) {
  await Promise.all([assertCollectionExists(COLLECTIONS.coupons), assertCollectionExists(COLLECTIONS.userCoupons)])
  const existing = await safeGetAll(COLLECTIONS.coupons)
  const byCode = new Map(existing.map((item) => [text(item.code).toUpperCase(), item]))
  let created = 0
  let skipped = 0
  for (const template of RECOMMENDED_COUPON_TEMPLATES) {
    if (byCode.has(template.code)) { skipped += 1; continue }
    const id = createId('coupon')
    await db.collection(COLLECTIONS.coupons).doc(id).set({
      ...template,
      couponType: 'fixed',
      startsAt: '',
      endsAt: '',
  ...
  return { created, skipped, total: RECOMMENDED_COUPON_TEMPLATES.length }
}

async function issueAutoCouponForUser(userId, trigger) {
  if (!userId) return
  const [templates, issued] = await Promise.all([
    safeGetAll(COLLECTIONS.coupons),
    safeGetAll(COLLECTIONS.userCoupons)
  ])
  for (const template of templates.filter((item) => item.enabled === true && text(item.autoTrigger, 'none') === trigger).slice(0, 10)) {
    await issueCouponDocument(template, userId, trigger, issued).catch(() => {})
  }
}

async function markOrderCouponUsed(order, orderId) {
  const userCouponId = text(order.userCouponId)
  if (!userCouponId) return
  const coupon = firstDocument(await db.collection(COLLECTIONS.userCoupons).doc(userCouponId).get())
  if (!coupon) return
  await db.collection(COLLECTIONS.userCoupons).doc(userCouponId).update({
    status: 'used',
    usedOrderId: orderId,
    usedAt: new Date(),
    lockOrderId: '',
...
async function releaseOrderCouponAdmin(order, orderId) {
  const userCouponId = text(order.userCouponId)
  if (!userCouponId) return
  const coupon = firstDocument(await db.collection(COLLECTIONS.userCoupons).doc(userCouponId).get())
  if (!coupon || text(coupon.lockOrderId) !== orderId) return
  const expired = dateValue(coupon.expiresAt) && dateValue(coupon.expiresAt) <= Date.now()
  await db.collection(COLLECTIONS.userCoupons).doc(userCouponId).update({
...
async function rejectOrder(event, adminContext) {
  ...
  await releaseOrderCouponAdmin(order, id).catch(() => {})
...
async function markOrderPaid(event, adminContext) {
  ...
  await markOrderCouponUsed(order, id).catch((error) => {
    console.warn('[adminApi] 优惠券核销失败：', error)
  })
...
async function startDelivery(event, adminContext) {
  ...
  const logisticsCompanyCode = 'shunfeng'
  const logisticsCompanyName = '顺丰速运'
  const trackingNo = text(event.trackingNo).replace(/\s+/g, '').toUpperCase()
  const note = text(event.note, '鲜花已经交由顺丰配送')

  if (!/^[A-Z0-9-]{8,32}$/.test(trackingNo)) {
    throw new BusinessError('请输入正确的顺丰运单号（8—32 位字母、数字或短横线）')
  }
  ...
  await db.collection(COLLECTIONS.orders).doc(id).update({
    ...
    trackingNo,
    logisticsProvider: 'sf-official',
    logisticsState: trackingNo === text(order.trackingNo).toUpperCase() ? text(order.logisticsState, '0') : '0',
    logisticsStateLabel: trackingNo === text(order.trackingNo).toUpperCase() ? text(order.logisticsStateLabel, '运输中') : '等待顺丰更新',
    logisticsTrace: trackingNo === text(order.trackingNo).toUpperCase() && Array.isArray(order.logisticsTrace) ? order.logisticsTrace : [],
    logisticsQueryError: '',
    logisticsUpdatedAt: trackingNo === text(order.trackingNo).toUpperCase() ? now : null,
...
async function completeOrder(event, adminContext) {
  ...
  await issueAutoCouponForUser(text(order.userId), 'orderCompleted').catch((error) => {
    console.warn('[adminApi] 复购券发放失败：', error)
  })
...
async function cancelAdminOrder(event, adminContext) {
  ...
  await releaseOrderCouponAdmin(order, id).catch((error) => {
    console.warn('[adminApi] 优惠券返还失败：', error)
  })
...
async function dashboard() {
  ...
  return {
    ...
    calendarEventCount: mergeCalendarEvents(calendarOverrides)
      .filter((item) => item.enabled !== false && text(item.region) !== 'merchant').length,
...
exports.main = async (event = {}) => {
  try {
    ...
    switch (action) {
      ...
      case 'listCoupons': return success(await listCoupons())
      case 'seedCoupons': return success(await seedCoupons(event, adminContext))
      case 'saveCoupon': return success(await saveCoupon(event, adminContext))
      case 'deleteCoupon': return success(await deleteCoupon(event))
      case 'issueCoupon': return success(await issueCoupon(event, adminContext))
