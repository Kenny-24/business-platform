...
async function buildProductSnapshots(rawItems) {
  ...
  for (const requested of items) {
    ...
    snapshots.push({
      ...
      colorTags: stringArray(product.colorTags),
...
function couponStatus(userCoupon, now = Date.now()) {
  const status = text(userCoupon && userCoupon.status, 'unused')
  const expiresAt = couponTimestamp(userCoupon && userCoupon.expiresAt)
  const validFrom = couponTimestamp(userCoupon && userCoupon.validFrom)
  const lockExpiresAt = couponTimestamp(userCoupon && userCoupon.lockExpiresAt)
  if (status === 'void' || status === 'used') return status
...
function couponApplicableAmount(coupon, items = []) {
  const included = stringArray(coupon && coupon.applicableCategories)
  const excluded = stringArray(coupon && coupon.excludedCategories)
  const excludedCampaignTypes = stringArray(coupon && coupon.excludedCampaignTypes)
  return items.reduce((sum, item) => {
    const keys = [text(item.type), text(item.category)].filter(Boolean)
    const includeMatched = !included.length || keys.some((key) => included.includes(key))
    const excludedMatched = keys.some((key) => excluded.includes(key))
    const campaignExcluded = excludedCampaignTypes.includes(text(item.campaignType))
    if (!includeMatched || excludedMatched || campaignExcluded) return sum
    return sum + integer(item.subtotalFen)
  }, 0)
}

function couponPublicView(coupon, items = [], now = Date.now(), context = {}) {
  const applicableAmountFen = couponApplicableAmount(coupon, items)
  const thresholdFen = integer(coupon.thresholdFen)
  const discountFen = Math.min(integer(coupon.discountFen), applicableAmountFen)
  const currentStatus = couponStatus(coupon, now)
  const firstOrderBlocked = coupon && coupon.firstOrderOnly === true && context.hasPaidOrder === true
  const canUse = currentStatus === 'unused' && !firstOrderBlocked && applicableAmountFen >= thresholdFen && discountFen > 0
  let unusableReason = ''
  if (firstOrderBlocked) unusableReason = '仅限首个已付款订单使用'
  else if (currentStatus === 'expired') unusableReason = '已过期'
  else if (currentStatus === 'used') unusableReason = '已使用'
  else if (currentStatus === 'locked') unusableReason = '已被其他订单占用'
  else if (currentStatus === 'notStarted') unusableReason = '尚未生效'
  else if (applicableAmountFen < thresholdFen) unusableReason = `还差¥${formatFen(thresholdFen - applicableAmountFen)}可用`
  else if (!discountFen) unusableReason = '当前商品不适用'

  return {
    _id: text(coupon._id),
    couponId: text(coupon.couponId),
    name: text(coupon.name, '优惠券'),
    code: text(coupon.code),
    thresholdFen,
    discountFen: integer(coupon.discountFen),
    thresholdText: thresholdFen > 0 ? `满¥${formatFen(thresholdFen)}可用` : '无门槛',
    discountText: formatFen(coupon.discountFen),
    validFrom: isoDate(coupon.validFrom),
    expiresAt: isoDate(coupon.expiresAt),
    applicableCategories: stringArray(coupon.applicableCategories),
    excludedCategories: stringArray(coupon.excludedCategories),
    firstOrderOnly: coupon && coupon.firstOrderOnly === true,
    applicableAmountFen,
    canUse,
    unusableReason,
...
async function userHasPaidOrder(userId) {
  try {
    const statusChecks = await Promise.all(
      ['making', 'delivering', 'completed'].map((status) =>
        db.collection(COLLECTIONS.orders).where({ userId, status }).limit(1).get()
      )
    )
    if (statusChecks.some((result) => (result.data || []).length > 0)) return true

    const paymentChecks = await Promise.all(
      ['paid', 'offlinePaid'].map((paymentStatus) =>
        db.collection(COLLECTIONS.orders).where({ userId, paymentStatus }).limit(1).get()
      )
    )
    return paymentChecks.some((result) => (result.data || []).some((order) => !['cancelled', 'refunded'].includes(text(order.status))))
  } catch (error) {
...
async function checkoutCouponViews(userId, items = []) {
  const now = Date.now()
  const documents = await listUserCouponDocuments(userId)
  const hasFirstOrderCoupon = documents.some((item) => item.firstOrderOnly === true)
  const hasPaidOrder = hasFirstOrderCoupon ? await userHasPaidOrder(userId) : false
  const expired = documents.filter((item) => text(item.status, 'unused') === 'unused' && couponStatus(item, now) === 'expired')
  for (const item of expired.slice(0, 20)) {
    db.collection(COLLECTIONS.userCoupons).doc(text(item._id)).update({ data: { status: 'expired', updatedAt: db.serverDate() } }).catch(() => {})
  }
  return documents
    .map((item) => couponPublicView(item, items, now, { hasPaidOrder }))
    .sort((a, b) => {
      if (a.canUse !== b.canUse) return a.canUse ? -1 : 1
      const discountDiff = b.discountFen - a.discountFen
      if (discountDiff) return discountDiff
      return couponTimestamp(a.expiresAt) - couponTimestamp(b.expiresAt)
    })
}

async function resolveSelectedCoupon(userId, couponId, items = []) {
  const normalizedId = text(couponId)
  if (!normalizedId) return null
  let coupon = null
  try {
    coupon = await getDocument(COLLECTIONS.userCoupons, normalizedId)
  } catch (error) {
    if (isMissingCollectionError(error)) return null
    throw error
  }
  if (!coupon || text(coupon.userId) !== userId) throw new BusinessError('优惠券不存在', 'COUPON_NOT_FOUND')
  const hasPaidOrder = coupon.firstOrderOnly === true ? await userHasPaidOrder(userId) : false
  const view = couponPublicView(coupon, items, Date.now(), { hasPaidOrder })
  if (!view.canUse) throw new BusinessError(view.unusableReason || '该优惠券当前不可用', 'COUPON_NOT_AVAILABLE')
  return { document: coupon, view }
}

async function lockCoupon(userCouponId, userId, orderId) {
  if (!userCouponId) return
  const coupon = await getDocument(COLLECTIONS.userCoupons, userCouponId)
  if (!coupon || text(coupon.userId) !== userId || couponStatus(coupon) !== 'unused') {
    throw new BusinessError('优惠券状态已变化，请重新选择', 'COUPON_STATE_CHANGED')
  }
...
async function releaseOrderCoupon(order) {
  const userCouponId = text(order && order.userCouponId)
  if (!userCouponId) return
  const coupon = await getDocument(COLLECTIONS.userCoupons, userCouponId)
  if (!coupon || text(coupon.lockOrderId) !== text(order._id)) return
  const nextStatus = couponTimestamp(coupon.expiresAt) && couponTimestamp(coupon.expiresAt) <= Date.now() ? 'expired' : 'unused'
  await db.collection(COLLECTIONS.userCoupons).doc(userCouponId).update({
    data: {
      status: nextStatus,
      lockOrderId: '',
...
async function listCheckoutCoupons(event, identity) {
  await ensureUser(identity)
  const items = await buildProductSnapshots(event.items)
  return {
    items: await checkoutCouponViews(identity.userId, items)
  }
}
...
async function buildPreview(event, identity, requireAddress = false) {
  ...
  const selectedCoupon = await resolveSelectedCoupon(
    identity.userId,
    text(event.couponId),
    items
  )
  const discountFen = selectedCoupon
    ? Math.min(integer(selectedCoupon.view.discountFen), integer(selectedCoupon.view.applicableAmountFen))
    : 0
  ...
  return {
    ...
    deliveryMethod: {
      ...deliveryMethod,
      feeText: deliveryMethod.id === 'pickup'
        ? '无需配送费'
        : deliveryFeeFen > 0
    ...
    selectedCoupon: selectedCoupon ? selectedCoupon.view : null,
    amounts: {
      goodsAmountFen,
      deliveryFeeFen,
      discountFen,
...
async function getCheckoutOptions(identity, event = {}) {
  ...
  const items = Array.isArray(event.items) && event.items.length
    ? await buildProductSnapshots(event.items)
    : []
  const coupons = items.length ? await checkoutCouponViews(identity.userId, items) : []

  return {
    deliveryMethods: DELIVERY_METHODS,
    pickupLocations: await listPickupLocations(),
    deliverySlots: DELIVERY_SLOTS,
    deliveryConstraints: items.length ? deliveryConstraints(items) : { earliestAt: new Date(Date.now() + 2 * 60 * 60 * 1000).toISOString(), minDate: chinaDateString(new Date(Date.now() + 2 * 60 * 60 * 1000)), maxDate: addChinaDays(chinaDateString(), 30) },
    coupons,
    recommendedCouponId: (coupons.find((item) => item.canUse) || {})._id || '',
    maxDeliveryDays: 365
  }
}
...
async function createOrder(event, identity) {
  ...
  const document = {
    ...
    userCouponId: preview.selectedCoupon && preview.selectedCoupon._id || '',
    couponId: preview.selectedCoupon && preview.selectedCoupon.couponId || '',
    couponName: preview.selectedCoupon && preview.selectedCoupon.name || '',
    couponCode: preview.selectedCoupon && preview.selectedCoupon.code || '',
  ...
  if (preview.selectedCoupon && preview.selectedCoupon._id) {
    await lockCoupon(preview.selectedCoupon._id, identity.userId, orderId)
  }
  ...
  try {
    ...
    if (preview.selectedCoupon && preview.selectedCoupon._id) {
      await releaseOrderCoupon({ _id: orderId, userCouponId: preview.selectedCoupon._id }).catch(() => {})
    }
    throw error
  }
...
function orderView(order, urlMap) {
  ...
  return {
    ...
    logisticsProvider: text(order.logisticsProvider, order.trackingNo ? 'sf-official' : ''),
    logisticsCompanyCode: text(order.logisticsCompanyCode, order.trackingNo ? 'shunfeng' : ''),
    logisticsCompanyName: text(order.logisticsCompanyName, order.trackingNo ? '顺丰速运' : ''),
    ...
    address: order.address
      ...
      : null,
    coupon: text(order.userCouponId) ? {
      userCouponId: text(order.userCouponId),
      couponId: text(order.couponId),
      name: text(order.couponName, '优惠券'),
      code: text(order.couponCode),
...
function sfApiConfig() {
  const partnerID = text(process.env.SF_PARTNER_ID)
  const checkWord = text(process.env.SF_CHECK_WORD)

  // Chloris 仅连接顺丰正式环境，避免真实运单误发往沙箱。
  // 不再读取 SF_API_ENV，也不接受沙箱地址覆盖。
  const productionUrl = 'https://bspgw.sf-express.com/std/service'
  ...
  return {
    ...
    url: productionUrl,
    configured: Boolean(partnerID && checkWord)
  }
}

function createSfDigest(msgData, timestamp, checkWord) {
  const source = encodeURIComponent(`${msgData}${timestamp}${checkWord}`)
  return crypto.createHash('md5').update(source, 'utf8').digest('base64')
}

function sfRouteState(routes = []) {
  const joined = routes.map((item) => `${text(item.remark)} ${text(item.opCode)}`).join(' ')
  if (/签收|妥投|已收件|已取件/.test(joined)) return '3'
  if (/派送|派件|投递/.test(joined)) return '5'
  if (/异常|拒收|退回|退件|无法派送|滞留/.test(joined)) return '2'
  if (/揽收|已收取|收取快件|已接收/.test(joined)) return '1'
  return '0'
}

function normalizeSfRoutes(routes = []) {
  return (Array.isArray(routes) ? routes : [])
    .map((item, index) => ({
      id: `${text(item.acceptTime)}_${text(item.opCode)}_${index}`,
      time: text(item.acceptTime),
      context: text(item.remark, '物流状态已更新'),
      status: text(item.opCode),
      statusCode: text(item.opCode),
      location: text(item.acceptAddress)
    }))
    .filter((item) => item.time || item.context)
    .sort((a, b) => {
      const left = new Date(a.time.replace(/-/g, '/')).getTime() || 0
      const right = new Date(b.time.replace(/-/g, '/')).getTime() || 0
      return right - left
    })
    .slice(0, 40)
}

async function querySfRoutes({ trackingNo, phoneSuffix }) {
  const config = sfApiConfig()
  if (!config.configured) {
    return {
      configured: false,
      message: '顺丰正式环境尚未配置，请设置 SF_PARTNER_ID 和 SF_CHECK_WORD'
    }
  }
  ...
  if (phoneSuffix) payload.checkPhoneNo = phoneSuffix

  const msgData = JSON.stringify(payload)
  const response = await postForm(config.url, {
    partnerID: config.partnerID,
    requestID,
    serviceCode: 'EXP_RECE_SEARCH_ROUTES',
    timestamp,
    msgDigest: createSfDigest(msgData, timestamp, config.checkWord),
    msgData
  })

  if (text(response.apiResultCode) !== 'A1000') {
    throw new Error(text(response.apiErrorMsg, '顺丰接口请求失败'))
  }

  const apiResult = parseJsonObject(response.apiResultData)
  if (apiResult.success !== true) {
    throw new Error(text(apiResult.errorMsg, '顺丰暂未返回物流轨迹'))
  }

  const resultMsg = parseJsonObject(apiResult.msgData)
  const routeResponses = Array.isArray(resultMsg.routeResps) ? resultMsg.routeResps : []
  const routeResponse = routeResponses.find((item) => text(item.mailNo) === trackingNo) || routeResponses[0] || {}
  const rawRoutes = Array.isArray(routeResponse.routes) ? routeResponse.routes : []
  const routes = normalizeSfRoutes(rawRoutes)
  const state = sfRouteState(rawRoutes)

  return {
    configured: true,
    routes,
    state,
    stateLabel: logisticsStateLabel(state),
    message: routes.length ? '顺丰物流已更新' : '顺丰暂未返回物流轨迹',
    requestID
  }
}
...
async function refreshLogistics(event, identity) {
  ...
  const num = text(order.trackingNo).replace(/\s+/g, '')
  if (!num) throw new BusinessError('商家尚未填写顺丰运单号')
  ...
  const cacheTtl = cachedTrace.length ? 15 * 60 * 1000 : 5 * 60 * 1000
  if (!force && lastUpdated && Date.now() - lastUpdated < cacheTtl) {
    return {
      configured: true,
      cached: true,
      provider: 'sf-official',
      logisticsCompanyCode: 'shunfeng',
      logisticsCompanyName: '顺丰速运',
      trackingNo: num,
      logisticsState: text(order.logisticsState, cachedTrace.length ? '0' : ''),
      logisticsStateLabel: text(order.logisticsStateLabel, cachedTrace.length ? '运输中' : '等待顺丰更新'),
      logisticsTrace: cachedTrace,
      message: cachedTrace.length ? '已显示最近一次顺丰物流结果' : text(order.logisticsQueryError, '顺丰暂未返回物流轨迹')
    }
  }

  const phoneSuffix = normalizePhoneSuffix(order.address && order.address.phone)
  let result
  try {
    result = await querySfRoutes({ trackingNo: num, phoneSuffix })
  } catch (error) {
    const hint = phoneSuffix
      ? ''
      : '；当前订单缺少收件手机号后4位，请检查收货地址信息'
    const message = `${error.message || '顺丰物流查询失败'}${hint}`
    await db.collection(COLLECTIONS.orders).doc(id).update({
      data: {
        logisticsProvider: 'sf-official',
        logisticsCompanyCode: 'shunfeng',
        logisticsCompanyName: '顺丰速运',
        logisticsQueryError: message,
  ...
  if (!result.configured) {
    return {
      ...
      trackingNo: num,
      logisticsState: text(order.logisticsState),
      logisticsStateLabel: text(order.logisticsStateLabel, '等待顺丰更新'),
      logisticsTrace: cachedTrace,
      message: result.message
    }
  }

  const trace = result.routes
  const state = result.state
  const stateLabel = result.stateLabel
  const queryError = trace.length ? '' : result.message
  ...
  await db.collection(COLLECTIONS.orders).doc(id).update({
    data: {
      ...
      logisticsCourierInfo: null,
      logisticsQueryError: queryError,
      logisticsUpdatedAt: db.serverDate(),
      updatedAt: db.serverDate()
    }
  })
...
async function cancelOrder(event, identity) {
  ...
  await releaseOrderCoupon({ ...order, _id: id }).catch((error) => {
    console.warn('[orderApi] 优惠券返还失败：', error)
  })
...
exports.main = async (event = {}) => {
  try {
    ...
    switch (action) {
      ...
      case 'listCheckoutCoupons':
        return success(await listCheckoutCoupons(event, identity))
      case 'previewOrder':
